      ...
      <div className='mt-4'>
        {order?.items?.map((item) => (
          <div key={item.productDetailId} className='flex items-center justify-between p-2 border-b'>
            ...
            <p className='text-blue-600 font-semibold'>{(item.discountPrice * item.quantity).toLocaleString()} VND</p>
          </div>
        ))}
      </div>
        ...
        {discountAmount > 0 ? (
          <>
            ...
            <div className='flex justify-end items-center gap-2 text-gray-600 text-lg'>
              ...
              <span className='text-blue-600 '>{finalAmount.toLocaleString()} VND</span>
            </div>
          </>
        ) : (
