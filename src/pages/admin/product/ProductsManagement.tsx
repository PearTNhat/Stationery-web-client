...
import ProgressBar from './addproduct/ProgressBar'
...
function ProductsManagement() {
      ...
      setProducts((prev) =>
        prev.map((p) =>
          p.productId === pId
            ? {
                ...p,
                productDetail: p.productDetail.filter((detail) => detail.productDetailId !== detailId),
                fetchColor: [
                  ...new Set(
                    p.productDetail
                ...
                minPrice:
                  p.productDetail.length > 1
                    ? Math.min(
                        ...p.productDetail
                          .filter((detail) => detail.productDetailId !== detailId)
                          .map((d) => d.discountPrice)
                      )
                    : 0,
                quantity: p.productDetail
                  .filter((detail) => detail.productDetailId !== detailId)
                  .reduce((sum, d) => sum + d.stockQuantity, 0),
                soldQuantity: p.productDetail
                  .filter((detail) => detail.productDetailId !== detailId)
                  .reduce((sum, d) => sum + d.soldQuantity, 0),
                totalRating:
                  p.productDetail.length > 1
                    ? p.productDetail
                        .filter((detail) => detail.productDetailId !== detailId)
                        .reduce((sum, d) => sum + d.totalRating, 0) /
  ...
  const handleConfirm = () => {
    const finalProduct: ListProductDetail = {
      ...
      productDetail: newProduct.productDetail.map((detail) => ({
        ...detail,
        productId: `p${products.length + 1}`
      })),
      img: newProduct.productDetail[0]?.images?.[0]?.url || ''
    }
    ...
    <div className='p-6 w-full mx-auto bg-white shadow-lg rounded-xl'>
      ...
      {step === 2 && (
        <AddProductDetailsForm
          productDetails={newProduct.productDetail}
          fetchColors={newProduct.fetchColor}
          onAddDetail={(detail) => {
            setNewProduct((prev) => {
              // Kiểm tra trùng lặp
              if (checkDuplicateDetail(detail, prev.productDetail)) {
                Swal.fire({
                  icon: 'error',
                  title: 'Lỗi',
              ...
              const newDetails = [...prev.productDetail, newDetailWithColorId]
          ...
          onUpdateDetail={(detail) => {
            setNewProduct((prev) => {
              ...
              const newDetails = prev.productDetail.map((d) =>
          ...
          onDeleteDetail={(detailId) => {
            setNewProduct((prev) => {
              const newDetails = prev.productDetail.filter((d) => d.productDetailId !== detailId)
          ...
          onFinish={() => {
            if (newProduct.productDetail.length === 0) {
              Swal.fire({
                icon: 'error',
                title: 'Lỗi',
            ...
            setProducts(
              products.map((p) =>
                p.productId === pId
                  ? {
                      ...p,
                      productDetail: p.productDetail.map((d) =>
